...
$$
where $\exp()$ is the matrix expoential. This follows from the so-called Kolmogorov forward equations, but again, for more details see @dobrow2016introduction. We can easily see that such a model now accounts for irregular observation times.
...
mllk = function(theta.star, timediff, x, N=2){
  ...
  diag(Q) = 0
  diag(Q) = -rowSums(Q)
  delta = solve(t(Q+1), rep(1,N)) # stationary distribution of the
