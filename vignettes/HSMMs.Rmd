...
We now want to briefly show the analysis of a real data set using hidden semi-Markov models. For this purpose we use the movement track of an Arctic muskox contained in the `R` package `PHSMM`. Originally these data where collected by ... and have already been analyzed by ...
...
t1 = Sys.time()
mod_muskox = stats::nlm(mllk_muskox, theta.star, step=data$step, N=3, agsizes=agsizes,
                        iterlim = 500)
Sys.time()-t1
```
