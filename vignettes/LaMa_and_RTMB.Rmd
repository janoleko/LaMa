...
Note that `delta`, `Gamma` and `allprobs` are always reported by default when using `forward()` which is very useful for e.g. state decoding, because many downstream `LaMa` functions take these arguments as inputs.
...
The model fit can then be conducted by using the `pql` function contained in `LaMa`, PQL standing for penalized quasi-likelihood. Under the hood, this function also constructs and AD function with `RTMB` but uses the PQL algorithm described in Koslik, 2024. We have to tell the `pql()` function which parameters are random/ spline coefficients by providing the name of the corresponding list element of `par`.
...
Most conveniently this is done by using the `dgmrf()` function included in `RTMB` which provides the density function of the multivariate normal distribution reparametrized in terms of the precision matrix, i.e. inverse covariance matrix, which in our case is $\lambda_i S$ for spline $i$.
`dgmrf()` expects a sparse precision matrix, hence he have to make `S` of class `sparseMatrix`.
...
dat$S = as(S[[1]], "sparseMatrix") # make sparse
```
...
pnll = function(par) {
  ...
  l = forward_g(delta, Gamma[,,tod], allprobs, ad = TRUE)
  
  lambda = exp(loglambda)
  l = l + dgmrf(betaspline[1,], 0, S, scale = 1 / sqrt(lambda[1]), log = TRUE) +
    dgmrf(betaspline[2,], 0, S, scale = 1 / sqrt(lambda[2]), log = TRUE)
  -l
}
```

```{r refit, echo = FALSE, message = FALSE, cache = TRUE}
obj4 = MakeADFun(pnll, par, random = "betaspline", silent = TRUE)
system.time(
  opt <- nlminb(obj4$par, obj4$fn, obj4$gr)
)
```

As we can see, the results are basically identical while the model fit took more than five times as long.
