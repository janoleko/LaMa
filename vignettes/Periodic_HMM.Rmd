...
This example shows how to fit an HMM, with the state process being a periodically inhomogeneous Markov chain. Formally, this means that for all $t$
...
```{r data}
# simulation
z = rep(1:48, 100) # time of day variable, 100 days
n = length(z)
set.seed(123)
s = x = rep(NA, n)
...
`Lcpp` also makes non-parametric modeling trivially easy. Here we model the transition probabilities using cyclic P-splines. We do so in first calculating the design matrix using `mgcv` which we can easily be handled by `tpm_p()`.

#### Building the cyclic spline design matrix

```{r cSpline}
nk = 12 # number of basis functions
tod = 1:48
L = 48
