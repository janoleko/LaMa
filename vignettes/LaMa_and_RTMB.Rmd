...
The workflow with `RTMB` is basically always the same. We need to define the negative log-likelihood function, create an automatically differentiable objective function from it and fit the model by numerical minimization of the latter. `RTMB` also provides many functions that make this process very convenient.

### Simple HMM

We start by fitting a super simple stationary HMM with state-dependent gamma distributions for the step lengths and von Mises distributions for the turning angles. As a first step, we define the initial parameter list `par` and a `dat` list that contains everything else. The names `par` and `dat` are of course arbitrary.
...
We see that `par` is a named list or initial parameter values which is really useful for accessing the parameters later on and much more convenient than indexing. With `RTMB`, you could also use a parameter vector, but using a named list is so much easier. Moreover, I use the `dat` list to package all the data and other hyperparameters that are not to be estimated, but this is just my choice out of convenience.
...
-   The `getAll()` function is extremely useful and you should use it in the first line to unpack both the `par` and the `dat` list, making all elements available without the `$` operator. We also see, that `mllk` just takes the dat object from the global environment.
-   Parameter transformations are of course still necessary, i.e. all parameters in `par` should be unconstrained.
-   Simple `LaMa` functions (like `tpm()` or `stationary()`) work for both automatic differentiation and standard optimization. For the more complicated functions like `forward()`, currently you need to specify `ad = TRUE` to get automatic differentiation. I would like to make this smoother in the future, but currently that's where we're at. Whenever you are not sure if you need `ad = TRUE`, just check the documentation.
-   You might wonder how on earth `RTMB` can calculate the gradient of distributions like the gamma or von Mises distribution. The answer is: It can't but provides its own version of all standard distributions like `dnorm()`, `dbinom()`, etc. In this case both `dgamma2()` and `dvm()` come from `LaMa` as these are non-standard, but under the hood build on `RTMB` functions. `dgamma2()` is just a convenience function as it reparametrizes the gamma distribution in terms of mean and standard deviation, which is often more intuitive.
- Actually, most of the standard functions (e.g. `sum()`), operators (e.g. `%*%`) and methods (e.g. `matrix`) are overwritten when you use `RTMB` but you typically don't notice that and should not care -- but this is how the magic works.
-   the `REPORT()` function offered by `RTMB` is insanely convenient as any quantities calculated in the likelihood function (for which you have the code anyway), if reported, will be available after optimization, while the report statements are ignored during optimization. So no annoying backtransformations anymore, wohoo!
- for simple parameter transformations, `ADREPORT()` is also great, because it calculates standard deviations for whatever quantities, based on the delta method. Note that the delta method is not advisable for complex non-linear and multivariate transformations.

Having defined the negative log-likelihood, we can now create the objective function and fit the model. This needs some explanation: At this point, `RTMB` takes the negative log-likelihood function and generates its own version of it, including a gradient. `MakeADFun()` now also grabs whatever is saved as `dat` in the global environment and *bakes* it into the objective function. Therefore, changes to `dat` after this point will have no effect on the optimization result. We set `silent = TRUE` to suppress printing of the optimization process.
...
names(obj)
```
We see that it contains the initial parameter `par` (now tranformed to a vector), the objective function `fn` (which in this case evaluate `mllk` but faster), its gradient `gr` and Hessian `he`.
...
obj$gr()
```
I do not show the Hessian for brevity.

We can now maximize the objective function. The optimization routine `nlminb` is very robust and conveniently allows us to provide a gradient function. Alternatively, you can also use `optim()`. Indeed, we do not provide the Hessian to `nlminb` because while evaluating the Hessian is very fast with `RTMB`, optimization is still much faster if we use a quasi-Newton algorithm that approximates the current Hessian based on previous gradient evaluations, than using full Newton-Raphson.
```{r modelfit}
...
opt$objective
```
Note that the naming here is determined by `nlminb`. If you use a different optimizer, these may be called differently.

Much nicer however is that `obj` (yes `obj` not `opt`) is automatically updated after the optimization. Note that calling `obj$gr()` after optimization now gives the gradient at the optimum, while `obj$fn()` still gives the objective at the starting value and `obj$par` is not updated but still the initial parameter vector (kind of confusing).

To get our estimated parameters on their natural scale, we don't have to do the backtransformation manually. We can just do the reporting:
...
Lastly, we can also use the `sdreport()` function to directly give us standard errors for our unconstraint parameters and everything we `ADREPORT()`ed. 
```{r sdreport}
sdr = sdreport(obj)
```
...
summary(sdr)
```
To get the estimated parameters or their standard errors in a list format, type
```{r sdreport3, eval = F}
...
as.list(sdr, "Std")
```
and to get the estimates and standard errors for `ADREPORT()`ed quantities only, type
```{r sdreport4, eval = F}
...
Generalizing to covariate effects is also straightforward. For example, we can add time of day variation to the state process. In this case we want to obtain a state process model of the form
$$
...
For this we compute the trigonometric basis design matrix `Z` corresponding to above predictor and add the time of day to the `dat` list. The `LaMa` function `trigBasisExp()` does this very conveniently.
...
mllk2 = function(par) {
  getAll(par, dat) # makes everything contained available without $
  Gamma = tpm_g(Z, beta, ad = TRUE) # computes covariate-dependent tpms (in this case only 24 unique)
  ADREPORT(Gamma) # adreporting
  Delta = stationary_p(Gamma, ad = TRUE) # periodically stationary dist
  ADREPORT(Delta)
  delta = Delta[tod[1],]
  # exponentiating because all parameters strictly positive
  ...
  -forward_g(delta, Gamma[,,tod], allprobs, ad = TRUE) # indexing 24 unique tpms by tod in data
}
```
...
And we can look at the reported results. In this case, I get standard errors for `Gamma` with the delta method because of simplicity. In general this is not advisable.
...
modmat = make_matrices(~ s(tod, bs = "cp"), 
                       ...
                       knots = list(tod = c(0,24))) # telling mgcv where to wrap
...
pnll = function(par) {
  getAll(par, dat) # makes everything contained available without $
  Gamma = tpm_g(Z, cbind(beta0, betaspline), ad = TRUE); ADREPORT(Gamma)
  Delta = stationary_p(Gamma, ad = TRUE); ADREPORT(Delta)
  delta = Delta[tod[1],]
  # exponentiating because all parameters strictly positive
  mu = exp(logmu); REPORT(mu)
  sigma = exp(logsigma); REPORT(sigma)
...
3. The penalty strength vector `lambda` needs its length to correspond to the *total* number of random effects used. In our case, this is the number of rows of betaspline, but if we had another random effect in our parameter list, we would have needed more elements in `lambda`.
...
5. When we summarise multiple spline coefficients in a matrix in our parameter list -- which is very useful when these are of same lengths and have the same penalty matrix -- this matrix must be arranged by row, i.e. each row is one spline coefficient vector. If it is arranged by column, `pql()` will fail.
...
```
The `mod` object is now a list that contains everything that is reported by the likelihood function, but also the `RTMB` object created in the process. I now ignore confidence bands because I'm lazy. After fitting the model, we can also use the `LaMa` function `pred_matrix()`, that takes the `modmat` object we created earlier, to build a new interpolating design matrix using the exact same basis expansion specified above. This allows us to plot the estimated transition probabilities as a smooth function of time of day.
...
Lastly, we could have achieved the fit above using the slightly more accurate full Laplace approximation method which can be used to integrate out the random effects which is natively supported by `RTMB` -- and actually one of its core selling points and the standard way we can now deal with random effects. Indeed, the PQL algorithm above treats the spline coefficients as multivariate Gaussian random effects but exploits their relatively simple structure. The full Laplace method is much more general, allowing for very flexible random effects, however, estimation will be slower because it does not exploit the simple structure of splines when they are treated as random effects.

We have to alter our likelihood function slightly, because for the Laplace method, we need the joint likelihood of the data and the random effect, the latter having a multivariate normal distribution in this case Specifically, if $b$ is our random effect for the spline, $b \sim N(0, \lambda^{-1} S^-)$. The likelihood of the data given $b$ is $f(x \mid b)$ (just our regular likelihoot that treats $b$ as a parameter) and the density of $b$ is $f_{\lambda}(b)$. Hence the joint likelihood is
$$
f(x, b) = f(x \mid b) f_{\lambda}(b)
$$
and the negative log-likelihood becomes $- \log(f(x \mid b)) - \log(f_{\lambda}(b))$. This is what we implement below.

Most conveniently this is done by using the `dgmrf2()` function included in `LaMa` which provides the density function of the multivariate normal distribution reparametrized in terms of the (scaled) precision matrix, i.e. inverse covariance matrix, which in our case is $\lambda_i S$ for spline $i$. It allows evaluating at multiple points at once, each one possibly with its own penalty strength parameter `lambda`. It differs from `RTMB`'s `dgmrf()` by not expecting a *sparse* precision matrix and being more robust for rank-deficient penalty matrices, which are typical for penalized splines. I did implement `dgmrf2()` because I felt like `dgmrf()` was very inconvenient for spline models, which is not surprising as it was not designed for this, but for Gaussian-Markov random fields (big surprise).
...
dat$S = S[[1]] # dgmrf2() cannot work with lists (returned by mgcv per default)
```
...
f(x) = \int f(x, b) db.
$$
(actually the negative log of $f(x)$.

```{r refit, message = FALSE}
obj4 = MakeADFun(jnll, par, random = "loglambda", silent = F)
system.time(
  opt4 <- nlminb(obj4$par, obj4$fn, obj4$gr)
)
```

As we can see, the results are basically identical while the model fit took about ten times as long.
...
There are some problems with `RTMB` one has to keep in mind. They can be a bit annoying, but in my opinion the benefits of automatic differentiation far outweigh the drawbacks. I list the main ones I have encountered here. Please tell me if you encounter more.

A typical issue with `RTMB` is that some operators might need to be overloaded to allow for automatic differentiation. In typical model setups `LaMa` functions do this themselves, but if you go a very individualistic route and get an error like 
```{r error, echo = FALSE}
...
"[<-" <- ADoverload("[<-")
```
in the first line of your likelihood function. If the error still prevails also add
```{r overloading2}
"c" <- ADoverload("c")
...
Another common problem occurs when initiating objects with `NA` values and then trying to fill them with `numeric` values. This is because `NA` is logical which screws up the automatic differentiation due to the mismatching types. To avoid this, always initiate with `numeric` values or `NaN`. For example, don't do
```{r NA, eval = FALSE}
X = array(dim = c(1,2,3))
...
* you can use `if` statements in your likelihood function, but *not* on the parameter itself as this will obviously not be differentiable.
