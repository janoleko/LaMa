...
Importantly, you cannot use `if` or `max`/ `min` statements **on your parameter itself**, as these are not differentiable. If you do so, `RTMB` will fail and probably does not produce a helpful error message. The problem here results from `RTMB` building the *tape* (computational graph) of the function at the initial parameter. If you have `if` statements, then the resulting gradient will be different from the one at another parameter value. Often, you can remedy this behaviour by exploiting the fact that `abs()` is differentiable (in code). For example, you can create the differentiable `max` alternative:
```{r max2, eval = FALSE}
max2 = function(x,y){
  (x + y + abs(x - y)) / 2
}
