...
* The `tpm`family with
  ...
  + `tpm_phsmm()` for calculating the transition matrix of a             periodic-HSMM-approximating HMM.

* The `stationary()` family to compute stationary and periodically stationary distributions.

* `trigBasisExp()` for efficient computation of trigonometric link functions.
