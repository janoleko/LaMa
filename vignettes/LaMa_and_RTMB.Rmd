...
For the purpose of this vignette, we will analyse the `elephant` data set contained in the `LaMa` package:
```{r data}
head(elephant, 5)
```
...
Generalising to covariate effects is also straightforward. For example, we can add time of day variation to the state process. In this case we want to obtain a state process model of the form
$$
