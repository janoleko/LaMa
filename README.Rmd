...
We analyse the `elephant` data set contained in the package using a simple 2-state HMM with state-dependent gamma distributions. We fit the model to the hourly step lengths.
...
After tranforming the unconstrained parameters to working parameters using `tpm()` and `stationary()`, we can visualise the results:
