...
In addition to providing fast and easy to use versions of the **forward algorithm**, this package is supposed to be a toolbox for flexible and fast model building. Thus, it contains more helpful functions for building HMM-like models. Currently these include

* `trigBasisExp()`: Efficient computation of trigonometric link functions
* `cont_tpm()`: Fast calculation of transition matrices of continuous-time HMMs
* `hsmm2hmm()`: Building the transition matrix for fitting HSMMs
* `phsmm2phmm()`: Building the transition matrix for fitting periodically inhomogeneous  HSMMs
...
Here we build the t.p.m. using the `tpm()` function and calculate the log-likelihood using `forward()` in the last line.
```{r mllk}
